...
| **T14** ✅ *(répondu)* | `GET /recipes` | **Réponse reçue : la photo est la clé `shop_photo_path`, et c'est un chemin, pas une URL.** Reste à confirmer contre quelle base le résoudre — même hôte que l'API, ou serveur de médias distinct | Branché : `ProductRepository::all()` lit `/recipes` et résout `shop_photo_path` contre le réglage `product_ref_photo_base` |
